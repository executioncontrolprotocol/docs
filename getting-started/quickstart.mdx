...
Runnable examples live in the [core examples](https://github.com/executioncontrolprotocol/executioncontrolprotocol/tree/main/examples) tree (echo, accepts/returns, secrets bind, encode/decode, test sessions, invoke).
...
`--dry-run` runs the same input check without invoking capabilities. `ecp validate` still checks the graph and environment only. See [Fluent API](/guides/fluent-api) and [Schemas](/reference/schemas).
