...
## Host system configuration and the `security` mirror
...
The reference CLI requires a resolvable system config for **`ecp run`** and **`ecp validate`**. If neither project-local config, user **`~/.ecp/*`**, nor **`--config <path>`** resolves, the command fails so host policy is never skipped.

Typical subtrees under `security` include:

- **`security.models`** — `allowProviders`, `defaultProviders`, and **`allowedModels`** (per-provider map of model names policy permits). Wiring uses **`models.providers.<id>.supportedModels`** for what the host can run; effective use requires both **supported** and **allowed** alignment (see the protocol SPEC host section).
...
Manage policy from the CLI under **`ecp config security`** (run `ecp config security` with no args for the command list). Wiring uses **`ecp config add|get|update|remove --type tools|models|loggers|endpoints`**. **`ecp config plugins`** covers installs; **`ecp config secrets`** covers secret store and YAML helpers (`ecp config --help` for the full tree).
