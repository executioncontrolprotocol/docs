...
**`ecp run`** and **`ecp validate`** require a system config on disk. If **`--config <path>`** is set, that single file is loaded (no merge). Otherwise the CLI resolves project-local **`./ecp.config.yaml`** / **`./ecp.config.json`** and user **`~/.ecp/`** paths; when **both** exist, it **merges** them (global overrides local on overlapping keys). If **no** file is found, the command exits with an error.

**Layout:** wiring lives under **`models`** (per-provider **`supportedModels`** under **`models.providers`**), **`tools`**, **`loggers`**, **`agents`**, **`secrets`**, optional **`executors`** / **`memory`**, and **`plugins.installs`**. **Policy** lives under the top-level **`security`** object, which **mirrors** those areas (`security.models`, `security.tools`, `security.plugins`, …). For models, **`security.models.allowedModels`** is a per-provider map of model names policy permits (non-empty for each id in **`security.models.allowProviders`**). Include a top-level **`version`** field that matches the schema expected by your CLI (see the annotated example below). Release-to-release changes are summarized in the project [changelog](/reference/changelog).

Use system config to register MCP servers under **`tools.servers`**, model providers under **`models.providers`**, and to restrict what the runtime may load via **`security.*`**. Remove config files with **`ecp config reset`** (project scope), **`ecp config reset --global`**, or **`ecp config reset --config <path>`**. Inspect or update security with `ecp config security get`, `ecp config security models allowed-models …`, `ecp config security plugins update`, and the rest of `ecp config --help` (plugins installs, models, tools, loggers, secrets, …).
...
- [Quickstart](/getting-started/quickstart) — Install from npm and first run
- [Build from source](/getting-started/build-from-source) — Clone and compile the monorepo
